...
protocol ExecuteBackStack:class {
    
    func goBackToProfileStack()
}
        ...
        if self.post?.caption == "" && self.post?.caption == nil {
            ...
            if arrObj.count > 0 {
                ...
                if post?.photoURL == "" || post?.photoURL == nil {
                    ...
                    } else {self.pageControll.currentPage = 1
                        self.pageControll.isHidden = true
...
//        print(post?.photoURL)
        ...
        if post.isLiked {
            ...
            if(!justLoading){
                if let bigLikeImageV = likeImageView {
                    UIView.animate(withDuration: 0.2, delay: 0.05, usingSpringWithDamping: 1, initialSpringVelocity: 1, options: .allowUserInteraction, animations: {
    ...
    @IBAction func toggleSpottedOnProfileRepost(_ sender: Any) {
        ...
        if !taggedVisible {//show
            ...
            for (_,value) in post.tags{
                if let value = value as? [String:Any], let x = value["x"] as? CGFloat, let y = value["y"] as? CGFloat, let username = value["username"] as? String{
                    ...
                    let button = UIButton(frame: CGRect(x: x, y: y+200, width: labelSize.width + 30, height: 28))
                    ...
                    button.addGestureRecognizer(tapOnUsername)
    ...
    @objc func usernameTagPressed(_ sender: TaggedButtonUIGesture){
        ...
        API.User.getUser(withUsername: sender.data!) { (user) in
            if let user = user {
                ...
                let currentNavController = appDelegate.currentNavController
...
//        let maxTopBounds = min(fanMenu.bounds.height + infoView.bounds.height, postImageView.bounds.height - 20.0)
    ...
    @IBAction func ellipsisButtonPressed(_ sender: Any) {
        ...
        if user.isCurrentUser {
            ...
            alertController.addAction(UIAlertAction(title:"Archive", style: .default, handler: {(action) in
                ...
                DispatchQueue.main.async {
                    ...
                    API.Post.archive(postWithID: self.post!.postID, fromPostAuthor: self.post!.authorUID)
                    self.delegate?.goBackInStack(post: self.post!)
        ...
        alertController.addAction(UIAlertAction(title: "Share", style: .default, handler: { (action) in
            print("Should be sharing this post")
...
extension PostTableViewCell: UITextViewDelegate {
    ...
    func showPhotoOverlay() {
        ...
        if(tagView != nil){
            ...
            ellipsisButton.isHidden = true //this means that we are on repost
            spotView.isHidden = false
        }
...
extension PostTableViewCell:TagListViewDelegate {

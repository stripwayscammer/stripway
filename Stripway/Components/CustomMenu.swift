...
class CustomMenu: MacawView {
    ...
    let items = [
        ("exchange","official_dm", 0x059FF5),
        ("visa", "official_dm", 0x4ECD5E),
        ("wallet", "official_dm", 0x27A2E1),
        ("visa", "sharepost", 0x595A6C),
        ("money_box", "sharepost", 0x39579A)
    ]
...
class CustomMenuButtonScene {
    ...
    init(radius: Double) {
        openButtons = createMenuState(image: "black_circle1", radius: radius)
        closeButtons = createMenuState(image: "black_circle1", radius: radius, a: 0.7)
